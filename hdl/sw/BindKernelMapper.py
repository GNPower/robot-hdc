...
from Arguments import DATA_PATH, parser, ParseArgs_BindKernelMapper

def binary(num):
    return ''.join('{:0>8b}'.format(c) for c in struct.pack('!f', num))

def ElementMultiplication_F(hv_length: int = 4, hv_max: int = 4):
    num_inputs = 2
    ...
    sum_hv = [np.prod(input_hv) for input_hv in input_hvs]
...
def GenerateInputs(hv_length: int = 4, hv_max: int = 4, implementation: int = 1):
    return implementation_map[implementation](hv_length, hv_max)

if __name__ == "__main__":
    ParseArgs_BindKernelMapper()
    args = parser.parse_args()
    output = GenerateInputs(args.hv_length, args.hv_max, args.implementation)
